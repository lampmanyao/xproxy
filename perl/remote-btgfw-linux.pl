...
sub local_server_init {
	...
	epoll_ctl($epfd, EPOLL_CTL_ADD, $serverfd, EPOLLIN);
}
...
sub client_event {
	...
	if (!$blen) {
		...
		epoll_ctl($epfd, EPOLL_CTL_DEL, $clientfd, EPOLLIN);
		$$sock->close;
		return;
	}
	...
	if ($$client_state == EXMETHOD) {
		...
		my ($ver, $cmd, $rsv, $type) = unpack("C C C C", $plaindata);
		if ($ver != SOCKS5_VERSION) {
			print "not socks5. close socket: $clientfd\n";
			delete $socket_map{$clientfd};
			epoll_ctl($epfd, EPOLL_CTL_DEL, $clientfd, EPOLLIN);
		...
		if ($type == IPV4) {
			...
			epoll_ctl($epfd, EPOLL_CTL_ADD, $clientfd, EPOLLIN);
			...
			if (! $remote) {
				...
				my $bin = pack("C C C C C a$doname_len S",
						...
						1 + $doname_len + PORT_LEN, $doname, $port);
				$$sock->write($bin);
				delete $socket_map{$clientfd};
				epoll_ctl($epfd, EPOLL_CTL_DEL, $clientfd, EPOLLIN);
				$$sock->close;
			...
			$socket_map{$$remotefd} = \%sock;
			epoll_ctl($epfd, EPOLL_CTL_ADD, $$remotefd, EPOLLIN);
			...
			print "unsupport type: $type. close socket: $clientfd\n";
			delete $socket_map{$clientfd};
			epoll_ctl($epfd, EPOLL_CTL_DEL, $clientfd, EPOLLIN);
			$$sock->close;
			return;
...
sub remote_event {
	...
	if (! $blen) {
		...
		epoll_ctl($epfd, EPOLL_CTL_DEL, $remotefd, EPOLLIN);
		$$sock->close;
	} elsif ($blen < 0) {
		print "close socket: $remotefd\n";
		delete $socket_map{$remotefd};
...
sub main {
	...
	while (1) {
		my $events= epoll_wait($epfd, 128, 1000);
